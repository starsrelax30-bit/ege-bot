...
mk = ReplyKeyboardMarkup(keyboard=[
    ...
    [KeyboardButton(text="📚 Словарь"), KeyboardButton(text="❓ Помощь")]
...
async def check_ege(cb: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Следующий тест", callback_data="next_ege")],
...
async def check_quick(cb: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎯 Следующий", callback_data="next_q")],
        [InlineKeyboardButton(text="📊 Статистика", callback_data="show_stats")]
    ])
    await cb.message.answer("Что дальше?", reply_markup=kb)
    await cb.answer()
